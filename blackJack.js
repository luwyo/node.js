...
function getSumCards(someCards) {
  ...
  for (
    var i = 0, doMinus = false, haveA = false;
    i < someCards.length;
    i++ // doMinus - меняли стоимость туза
  ) {
    if (someCards[i] == "A" && sum + 11 <= 21 && !haveA) {
      ...
      /*someCards.charCodeAt(0) > 65*/
      sum = sum + 10;
    } else {
      sum = sum + parseInt(someCards[i]);
    }

    if (haveA && !doMinus && sum > 21) {
      // если был туз=11 и при добавлении еще одного случился перебор
...
function handStatus() {
  return (
    ...
    dealerCards.join(" ") +
    " сумма " +
    getSumCards(dealerCards) +
    "\nУ вас " +
    playerCards.join(" ") +
    " сумма " +
    getSumCards(playerCards)
  );
}

function playerMoreCards() {
  // добавляем карты если нужно игроку
  var playerNext = true;
  do {
    if (getSumCards(playerCards) < 21 && playerNext) {
      ...
      playerNext =
        1 ===
        Number(
          syncprompt("Ещё карту? (1 - взять ещё, что-либо другое - нет). ")
        );
      if (playerNext) {
...
function dealerMoreCards() {
  // добавляем карты если нужно дилеру
  var dealerNext = true;
  do {
...
if (argv.help /*|| argc<2*/) {
  rl.write(
    "\tUsage:\tnode blackJack.js [LogFile]\n\tWhere:\tLogFile - path to log file."
  );
  rl.close();
} else {
  if (argv.log) {
    ...
    if (!fs.statSync("./logs").isDirectory())
      fs.mkdir("./logs", function (err) {
        if (err) throw "Не могу создать директорию: " + err;
      });
  }
  if (argv._[0]) {
    doLog = true;
  ...
  while (moreGame) {
    ...
    if (playerSum > 21) {
      ...
      if (dealerSum > 21) {
        // мы уже проверили что у игрока сумма норм
        console.log(
    ...
    if (doLog) {
      if (fs.existsSync(logFile)) {
        if (fs.statSync(logFile).size > 0) fs.appendFileSync(logFile, ",");
      }
      fs.appendFileSync(
        logFile,
        dealerCards.join(" ") +
          ...
          playerCards.join(
            " "
          ) /*,function (err) {if (err) throw 'Не могу открыть для записи файл: '+err;}*/
      );
    }
    moreGame =
      1 ===
